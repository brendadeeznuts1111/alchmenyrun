 ...
 */
async function authenticateWithJWT(token: string, env: Env): Promise<AuthResult> {
 ...
 */
export async function generateJWT(payload: Record<string, unknown>, secret: string, expiresIn: number = 3600): Promise<string> {
  const header = {
    alg: 'HS256',
    typ: 'JWT'
  };
