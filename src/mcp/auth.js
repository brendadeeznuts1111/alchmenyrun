...
async function authenticateWithJWT(token, env) {
    ...
    try {
        ...
        const key = await crypto.subtle.importKey("raw", encoder.encode(env.MCP_JWT_SECRET), { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
        const signature = base64UrlDecode(signatureB64);
        const isValid = await crypto.subtle.verify("HMAC", key, signature, encoder.encode(dataToVerify));
        if (!isValid) {
            return {
                success: false,
