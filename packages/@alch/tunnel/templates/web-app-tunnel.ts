...
export async function webAppTunnel(config: WebAppTunnelConfig) {
  ...
  return {
    ...
    getApiUrl: (path = "") =>
      `https://${config.hostname}${config.apiPath || "/api"}${path}`,
    getLocalUrls: () => ({
      frontend: `${
        config.enableHttps ? "https" : "http"
      }://localhost:${config.frontendPort}`,
...
export async function example() {
  const { tunnel, getRunCommand, getFrontendUrl, getApiUrl, getLocalUrls } =
    await webAppTunnel({
      hostname: "app.example.com",
      frontendPort: 3000,
      apiPort: 8080,
