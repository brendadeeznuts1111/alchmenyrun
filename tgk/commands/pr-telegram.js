...
export async function handlePRCallback(callbackData) {
...
export async function sendPRToTelegram(prId, chatId) {
...
function buildPRKeyboard(prId, status) {
  const keyboard = [];
...
async function postPRActionConfirmation(report, pr) {
  const actionEmojis = {
    'approve': '✅',
    'request-changes': '🔄',
    'comment': '💬',
...
export async function answerPRQuestion(prId, question) {
...
async function generatePRAnswer(pr, question) {
